...
import { ApplicationStatusBadge } from "./dashboard/application-status-badge";
...
export function AtsScoreHeader({ score, scoreBreakdown, jobId }: AtsScoreHeaderProps) {
            ...
            <div className="flex items-center justify-between max-w-5xl mx-auto">
                <div className="flex items-center gap-4">
                    <div className="flex items-center gap-2">
                        <span className="text-sm font-medium text-muted-foreground uppercase tracking-wider">Job Match Score</span>
                        <TooltipProvider>
                            <Tooltip>
                                <TooltipTrigger>
                                ...
                                <TooltipContent>
                                    <p>Overall alignment between your resume and the target job description. Updates live as you tailor content and keywords.</p>
                                </TooltipContent>
                            </Tooltip>
                        </TooltipProvider>
                    </div>
                    {jobId && <ApplicationStatusBadge jobId={jobId} />}
                </div>
                ...
                <div className="flex items-center gap-6">
                    <div className="flex flex-col items-end">
                        ...
                        {scoreGain > 0 ? (
                            ...
                            <span className="text-xs text-amber-600 dark:text-amber-400 font-medium max-w-md text-right">
                                Baseline: {originalScore}/100 • Tailored: {score}/100 — Limited alignment with this role. Review gaps below and only add skills you genuinely have experience with.
                            </span>
                        ) : (
                            <span className="text-xs text-muted-foreground font-medium">
                                Baseline match: {originalScore}/100 • Tailored match: {score}/100
                            </span>
                        )}
                    </div>
                </div>
